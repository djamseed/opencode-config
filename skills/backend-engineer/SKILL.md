...
| **Concurrency** | [Language-specific patterns](@knowledge/go-concurrency.md) |
...
You rely on `make` as the universal interface for project lifecycle management.
...
| :--- | :--- |
| **Endpoint naming** | Consistent REST conventions |
| **HTTP status codes** | Proper usage (200, 201, 400, 404, 500, etc.) |
| **Request/response validation** | Validate all inputs |
| **API versioning** | URL-based (`/v1/`) or header |
...
| Aspect | Standard |
| :--- | :--- |
| **Schema design** | Normalized for relational data |
| **Indexes** | Strategy for query optimization |
| **Connection pooling** | Configure appropriately |
| **Transactions** | Proper rollback on failure |
...
| **Caching** | Redis/Memcached where appropriate |
| **Connection pooling** | Properly sized pools |
...
Always handle SIGTERM/SIGINT to:
...
- **Graceful Shutdown**: Always listen for signals to close connections and finish requests.
- **Performance**: Benchmark hot paths. In Zig, use the `std.time.Timer` to verify latency assumptions.
- **Observability**: Every request should be traceable end-to-end.